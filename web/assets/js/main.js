...
(function() {
  ...
  /**
   * Mobile nav toggle - Universal function for all pages
   */
  window.initMobileNavigation = function() {
    console.log('Initializing mobile navigation...');
    const mobileNavToggleBtn = document.querySelector('.mobile-nav-toggle');
    console.log('Mobile nav button found:', mobileNavToggleBtn);
    
    function mobileNavToogle() {
      console.log('Mobile nav toggle clicked');
      document.querySelector('body').classList.toggle('mobile-nav-active');
      
      // Toggle icon directly on the element (since it's an <i> tag now)
      if (mobileNavToggleBtn) {
        mobileNavToggleBtn.classList.toggle('bi-list');
        mobileNavToggleBtn.classList.toggle('bi-x');
        console.log('Icon toggled, current classes:', mobileNavToggleBtn.className);
      }
    }
    ...
    if (mobileNavToggleBtn) {
      ...
      newBtn.addEventListener('click', function() {
        console.log('Mobile nav toggle clicked (new listener)');
        document.querySelector('body').classList.toggle('mobile-nav-active');
        
        // Toggle icon directly on the element
        newBtn.classList.toggle('bi-list');
        newBtn.classList.toggle('bi-x');
        console.log('Icon toggled, current classes:', newBtn.className);
      });
      
      console.log('Event listener added to mobile nav button');
    } else {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    // Wait a bit for header to be loaded (if fetched)
    setTimeout(() => {
      const mobileNavToggleBtn = document.querySelector('.mobile-nav-toggle');
      if (mobileNavToggleBtn && typeof initMobileNavigation === 'function') {
        initMobileNavigation();
      }
    }, 200);
  });

  // Also try to initialize immediately if elements already exist
  const mobileNavToggleBtn = document.querySelector('.mobile-nav-toggle');
  if (mobileNavToggleBtn) {
    initMobileNavigation();
  }
